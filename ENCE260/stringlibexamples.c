...
size_t readMessage(FILE* file, char text[], size_t maxTextSize)
{
    ...
    if (strncmp(text, "\n", maxTextSize-1) == 0) {
        *copy = 97;
    } else if (strncmp(text, "\t", maxTextSize-1) == 0) {
        *copy = 96;
    } else if ((strncmp(text, " ", maxTextSize-1) >= 0) && (strncmp(text, "~", maxTextSize-1) <= 0)) {
        *copy -= 31;
    } else {
        ++copy;
        count++;
    ...
    while ((*text++ = *copy++)) {
        if (strncmp(text, "\n", maxTextSize-1) == 0) {
            *copy = 97;
        } if (strncmp(text, "\t", maxTextSize-1) == 0) {
            *copy = 96;
        } if ((strncmp(text, " ", maxTextSize-1) >= 0) && (strncmp(text, "~", maxTextSize-1) <= 0)) {
            *copy -= 31;
        } else {
            ++copy;
            count++;
