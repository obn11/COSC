...
class Scanner:
    ...
    def skip_white_space(self):
        ...
        while not done:
            ...
            elif self.input_string[self.current_char_index].isspace():
    ...
    def consume(self, *expected_tokens):
        ...
        if token[0] not in expected_tokens:
            self.unexpected_token(token[0], expected_tokens)
        else:
            if token[0] != 'ID' and token[0] != 'NUM':
                token = token[0]
            self.current_token = self.get_token()
            return token
...
class If_Else_AST:
    def __init__(self, condition, then, concequent):
        self.condition = condition
        self.then = then
        self.concequent = concequent
    def __repr__(self):
        return 'if ' + repr(self.condition) + ' then ' + \
                       repr(self.then) + 'else ' + repr(self.concequent) + ' end'
    def indented(self, level):
        return indent('If-Else', level) + \
               self.condition.indented(level+1) + \
               self.then.indented(level+1) + \
               self.concequent.indented(level+1)
    def code(self):
        l1 = label_generator.next()
...
def if_statement():
    ...
    if scanner.lookahead() == Token.ELSE:
        ...
        concequent = statements()
        scanner.consume(Token.END)
        return If_Else_AST(condition, then, concequent)
    scanner.consume(Token.END)
